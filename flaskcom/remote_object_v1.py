...
#                self.ClientWrapper_wrapped_class = line.split("from ")[1].split(" import ")[1]
        ...
        if start_server:
            ...
            if not isup:
                ...
                if wrapped_function != None:
                    python_command,env = self.ClientWrapper_get_python_command_for_wrapped_function(wrapped_function, server)
                    #env = None
                ...
                self.ClientWrapper_start_server(command, env)
    ...
    def ClientWrapper_get_python_command_for_wrapped_function(self, wrapped_function, server):
        ...
        flaskcom_env = os.environ.copy()
        flaskcom_env["FLASKCOM_WRAPPED_FUNCTION"] = base64_encoded_dilled_function
        flaskcom_env["FLASKCOM_PORT"] = str(self.ClientWrapper_port)
        flaskcom_env["FLASKCOM_SERVER"] = server
        import inspect
        ...
        command = "python -c 'import sys;sys.path.append(\\\""+self.ClientWrapper_flaskcom_path+"\\\");from flaskcom import function_decoder;function_decoder.start()'"
        return command,flaskcom_env
    ...
    def ClientWrapper_start_server(self,command,env = None):
        import time
        print("running:")
        print(command)
        print(env)
        terminal = subprocess.check_call(command, stdin=subprocess.PIPE, shell=True, env=env)
